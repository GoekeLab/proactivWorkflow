...
approaches are sample specific and reliable, such experimental data is most 
often not available. Another approach utilizes the entire set of annotated 
transcription start sites to identify promoters. However, this includes a fixed
set of promoters which is not sample specific, and the high number of promoters
considered means a loss in statistical power in downstream analyses.  
...
data [@Demircioğlu2019]. Here, we present a workflow for promoter analyis by 
...
```
workflowData/
├── TCGA_Kidney_Junction_Files/
├── Yan_Junction_Files/
├── condition.tcga.rds
├── condition.yan.rds
├── gencode.v19.annotation.gtf
├── gencode.v34.annotation.gtf
├── junctionFilePaths.tcga.rds
├── junctionFilePaths.yan.rds
├── promoterAnnotation.gencodev19.rds
├── promoterAnnotation.gencodev34.rds
...
`r Biocpkg("proActiv")` implements the junction reads method to identify active 
promoters. The absolute promoter activity of a promoter is proportional to the 
log of the total count of the junction reads aligning to the set of first 
introns belonging to the transcripts of the promoter. Junction read counts are 
normalized with `r Biocpkg("DESeq2")` [@Love2014] normalization, and log 
transformed to obtain absolute promoter activity. Gene expression is defined as 
the sum of absolute promoter activities for all promoters belonging to a gene. 
The relative promoter activity of a promoter is the promoter's absolute activity 
normalized by total gene expression.  
...
promoter counts and activity for annotatedpromoters. After identifying promoters
...
Alternatively, promoter annotations can be created from transcript database with 
the `txdb` argument. We construct the *TxDb* object from the GTF file with the 
*makeTxDbFromGFF* function from the `r Biocpkg("GenomicFeatures")` package 
[@Lawrence2013]. 
```{r prepare annotation from TxDb, eval=FALSE, message=FALSE, warning=FALSE}
...
################################# Remove this
```{r tmp1, eval=TRUE}
promoterAnnotation <- readRDS("workflowData/promoterAnnotation.gencodev34.rds")
```
...
The *intronRanges* slot is *GRanges* object comprising the range of each of 
first intron annotated with the corresponding transcript(s). The 
*promoterIdMapping* slot is a dataframe that gives a mapping between promoter, 
transcript (Ensembl ID) and gene (Ensembl ID). The *promoterCoordinates* slot is
a *GRanges* object describing the coordinates of each annotated promoter along
with a map to its associated introns, the end coordinate of the first exon 
associated with the promoter, and whether the promoter is internal or not. This
information is used downstream in the next step to estimate promoter activity at
each annotated promoter.

## Estimating promoter activity

In this step, promoter activity at each annotated promoter is estiamted.
...
Here, we demonstrate *proActiv* with six STAR junction files from the 
[SGNEx project](https://github.com/GoekeLab/sg-nex-data) from the A549 and HepG2
cell lines attached with the `r Biocpkg("proActiv")` package. We define a 
condition vector corresponding to the input files.
...
describes that position of the promoters (5' to 3') and whether the promoter is
internal or not. 
...
and $x_i=0$ if otherwise. The adjusted p-value of the coefficients $\beta_A$ 
and $\beta_R$ are used to determine if the promoter is significantly 
alternatively regulated or not. Only promoters that are active with absolute and
relative promoter activity estimates over a certain threshold are considered. To
identify promoters up-regulated in the reference condition, promoters that 
exhibit significant fold change in the reference condition are identified. 
Down-regulated promoters in the reference condition are identified based on 
up-regulated promoters in the other samples. In addition, to identify promoters 
that exhibit differential regulation independent of changes in gene expression, 
an upper bound is placed on the gene expression fold change. 
...
Alternative promoters are identified here with A549 cell line as the reference 
condition: 
...
The *plots* object, shown in stores the plot for absolute promoter activity, 
relative promoter activity and gene expression in the first, second and third 
slot respectively. Both figure \@ref(fig:plot1) and figure \@ref(fig:plot2) 
generated by *plotPromoters* and *boxplotPromoters* reflect the alternative 
usage of promoters 137 and 141 as called by *getAlternativePromoters*.

# Analysis of TCGA Kidney Renal Clear Cell Carcinoma data

We demonstrate the functionality of `r Biocpkg("proActiv")` by analysing RNA-seq 
data of Kidney Renal Clear Cell carcinoma (KIRC) samples from The Cancer Genome 
Atlas (TCGA) Program [@Network2013]. The data comprise 20 normal samples and 18 
tumour samples. The reference genome used in this analysis corresponds to the 
GENCODE Release 19 version.
...
Prepare the promoter annotation:
```{r prepare annotation 19, eval=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
...
################################# Remove this
```{r tmp2}
promoterAnnotation <- readRDS("workflowData/promoterAnnotation.gencodev19.rds")
```

Load the junction file paths, condition vector and run proActiv:
...
We visualize promoter usage of a known cancer marker CDK4 in \@ref(fig:plot3) 
which exhibits alternative promoter usage as identifed by 
...
is included in the data for ths worflow. The original study applied scRNA-seq 
...
scRNA-seq data does not compromise downstream analysis.   
In particular, we:

-  Keep genes that are expressed in at least half of the cells at each 
   developmental stage  
-  Keep cells that express more than 80% of genes detected above  
...
We can look at the contribution of each major promoter to the total expression
for each gene. Figure \@ref(fig:majorVsGene) shows the major promoter activity against
gene expression for embryos in the 8 cell stage. Single promoter genes lie on 
the diagonal, while multi-promoter genes lie to the right of the diagonal.
A single major promoter does not often fully explain gene expression, with 
minor promoters also contributing to gene expression.
```{r majorVsGene, fig.cap="Major promoter activity against gene expression in 8 cell embryos.", cache=TRUE, message=FALSE}
library(dplyr)
library(ggplot2)
...
clusters of cells by developmental stages (figure \@ref(fig:plotpca)). Cells from 
earlier stages of development (Oocyte, Zygote, 2-cell and 4-cell stages) form 
one major cluster, sharing similar promoter activity profiles. The next major 
cluster comprises cells from the 8-cell stage and Morulae, while Blastocysts 
appear to have promoter activity profiles distinct from cells in all other 
...
alternative promoters between these two developmental stages. This may identify
instances of alternative promoter usage involved in the MZT. 
```{r mzt}
...
in the 8 cell stage in figure \@ref(fig:compare4and8), coloured by their gene 
expression differences. We expect alternatively use promoters to lie away from 
the diagonal, while displaying similar levels of gene expression across 4 and 8
cell embryos. The promoters identified by *getAlternativePromoters* are labelled
here. 
```{r compare4and8, fig.cap="Comparison of 4 cell promoter activity and 8 cell promoter activity.", warning=FALSE, message=FALSE}
library(dplyr)
library(ggplot2)
library(ggrepel)
...
PIAS1 (ENSG00000033800.9) and HP1BP3 (ENSG0000127483.13) in 
figure \@ref(fig:plotPIAS1) and figure \@ref(fig:plotHP1BP3) respectively, 
identified with *getAlternativePromoters*.
...
The MZT is characterized by degradation of maternal mRNA and transcripts to 
begin the transcription of zygotic genes [@Schultz2002; @Lee2014]. Alternative 
promoter usage, as shown in PIAS1 and HP1BP3, could be involved in this process 
of mRNA degradation and silencing and for onset of zygotic transcription. For 
instance, the PIASy family of genes have been implicated in the proper induction 
of zygotic genome activation and embryonic development [@Higuchi2019]. In 
addition, HP1BP3 has 
been shown to promote global miRNA biogenesis in human cells [@Liu2016]. 
Alternative promoter usage in HP1BP3 between 4-cell and 8-cell embryos could be 
involved in miRNA production and silencing of maternal mRNA in this transition. 
While miRNA mediated decay of maternal transcripts has not been shown in the 
human MZT, this system is present in Drosophila and Zebrafish
[@Tadros2005; @Giraldez2006]. 
